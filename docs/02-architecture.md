...
A layered stack protects against the LLM context window filling up. Ordered from durable to per-call:

**ADK's `EventsCompactionConfig`** (`google/adk/apps/compaction.py`). adk-cc's `agent.py` constructs it from env (`ADK_CC_COMPACTION_TOKEN_THRESHOLD` + `ADK_CC_COMPACTION_EVENT_RETENTION` for token-threshold mode; `ADK_CC_COMPACTION_INTERVAL` + `ADK_CC_COMPACTION_OVERLAP` for sliding-window) and passes it to `App(events_compaction_config=...)`. ADK's runner triggers compaction post-invocation; `LlmEventSummarizer` handles the LLM call, with safe split logic that preserves function-call/response pairing and avoids compacting pending calls. One adk-cc patch on top: ADK counts *any* function_response as an answer, but a confirmation-gated call is closed by an interim `needs_confirmation` response while it waits for the user — so ADK would compact the parked call's event and the post-approval response would orphan ("No function call event found…"). `context/compaction_pin.py` (installed with the config) makes the pending-call computation name/payload-aware so parked confirmations survive compaction. A dedicated compaction model is supported via `ADK_CC_COMPACTION_MODEL` (+ optional `_API_BASE`/`_API_KEY`); when unset it falls back to the agent's main model.

**Precompact** (`plugins/precompact.py`). ADK's trigger reads the last event's model-reported usage, which goes stale exactly when a session is poisoned — so this plugin measures the events payload-inclusively at `before_run` and compacts an oversized inherited history *before* the turn's first model call, via the same summarizer stack (digest-before-summarize, mechanical model-free fallback in FORCE mode at the guard's reject line). The same core also powers two on-demand paths:

- **Mid-turn compaction** (`midturn_compact_prior`, kill switch `ADK_CC_MIDTURN_COMPACT=0`): when the guard's pressure line stays hot after request rewriting, PRIOR-invocation events are durably summarized in the middle of the turn so every subsequent call rebuilds smaller. Current-invocation events are never candidates.
- **Guided `/compact`** (`manual_compact`, `POST /api/compact`): user-initiated compaction of a quiescent session (409 while a turn runs), with an optional emphasis guide ("keep #127, drop finished tasks") appended to the summarizer prompt. Interactive summarizer cap 60s; mechanical fallback guaranteed.

All three direct-summarizer paths apply the #119 pending rule via `_clamp_head_pending` — an event carrying a call parked behind a confirmation is never folded.

**Microcompact** (`plugins/microcompact.py`, on by default, `ADK_CC_MICROCOMPACT=0` disables). Rewrites the OUTGOING request only (ADK rebuilds it from events each call, so stored history keeps full content). Three shapes: allowlisted `function_response` payloads, foreign-tool-result TEXT renderings, and fat string args of COMPLETED `function_call`s (a `write_file` body rides in the call, not the result; calls without a real answer are untouched). Prefers cached LLM summaries, falls back to stubs.

**`ContextGuardPlugin`** (`plugins/context_guard.py`) — the per-call safety net, `before_model_callback`. Always attached; no-ops when `ADK_CC_MAX_CONTEXT_TOKENS` is unset. It estimates the outgoing request payload-inclusively and **calibrates** the estimate per session against the API's `usage_metadata.prompt_token_count` (chars/4 undercounts real tokenizers; a mid-turn 128K→219K growth once sailed under the reject line). The ladder:

- **WARN** at `ADK_CC_CONTEXT_WARN_TOKENS` (default 75% of effective): structured log line.
- **PRESSURE** at `ADK_CC_CONTEXT_PRESSURE_PCT` (default 85%, 0 disables): rewrites the request aggressively (escalating `keep_recent` 2→1→0, targeting ~70%), and if still hot, triggers the mid-turn prior-invocation compaction above (once per invocation).
- **REJECT** at `ADK_CC_CONTEXT_REJECT_TOKENS` (default 95%): last-resort rewrite passes (down to keep-nothing desperation), then a friendly "context near full" `LlmResponse` instead of a model-server 500 — precompact FORCE recovers the session next turn.
