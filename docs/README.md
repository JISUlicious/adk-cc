...
- [`08-desktop-app.md`](./08-desktop-app.md) — the single-user desktop app (Tauri + Python sidecar): data dir, the `settings.env` config file, dev run modes, building the self-contained AppImage installer, and the relocatable path layout.
- [`09-building.md`](./09-building.md) — build + test runbook: what each artifact is built from, why `VITE_*` flags come from the REPO-ROOT `.env`, why `tsc --noEmit` is not the build check, how the three classes of test differ (unit / e2e / live), and the `.env` trap that breaks live tests in both directions.
