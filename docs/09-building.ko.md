...
뿐이다. 데스크톱 앱은 `dist-desktop`을, `ADK_CC_SERVE_UI=1`로 띄운 FastAPI
서버는 `dist`를 서빙한다. 공용 UI 코드를 고치고 한쪽만 다시 빌드하는 것이
낡은 번들을 테스트하게 되는 가장 흔한 경로다.
...
- `tests/e2e_*.py` — 실제 서버, 대개 실제 브라우저(Playwright). 대부분 모델 없이
  ...
  스킵한다.
- `ADK_CC_LIVE=1 tests/e2e_*.py` — 실제 모델 턴을 쓰는 일부. 엔드포인트에
  rate limit이 있으므로 opt-in이다. 간격을 두고 돌리고, 루프로 돌리지 않는다.
