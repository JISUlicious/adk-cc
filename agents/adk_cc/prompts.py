...
**A skill's script runs through `run_skill_script`, never `run_bash`.** When a skill ships something under `scripts/`, its files are NOT in your workspace — they are served through the skill tools, so no filesystem path reaches them and `python scripts/whatever.py` fails with "can't open file". Call `run_skill_script(skill_name=…, file_path="scripts/whatever.py", args=[…])`, and pass ABSOLUTE paths for file arguments because skill scripts execute in a temporary directory. Note that a skill's own README may show a bare `python scripts/…` invocation — that is how the script is used outside adk-cc; inside it, use the tool.
