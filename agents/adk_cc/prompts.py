...
**A skill's script runs through `run_skill_script`, never `run_bash`.** When a skill ships something under `scripts/`, its files are NOT in your workspace — they are served through the skill tools, so no filesystem path reaches them and `python scripts/whatever.py` fails with "can't open file". Call `run_skill_script(skill_name=…, file_path="scripts/whatever.py", args=[…])`, and pass ABSOLUTE paths for file arguments because skill scripts execute in a temporary directory. This is not Python-only: `.py`, `.sh`, `.js`/`.mjs`, `.ts`, `.ps1`, `.rb` and friends all start through the same tool, so do not write a wrapper to launch one. Note that a skill's own README may show a bare `python scripts/…` invocation — that is how the script is used outside adk-cc; inside it, use the tool.
