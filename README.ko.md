...
외부 MCP 서버를 coordinator에 연결합니다. 규모에 따라 세 가지 방법이 있으며, 모두 도구를 `mcp__<server_name>__*`로 노출하므로 권한 규칙이 서버 단위로 타겟팅할 수 있습니다(예: `mcp__github__*` deny).

**정적 서버 1개 (dev / 빠른 시작)** — env 변수:

```bash
ADK_CC_MCP_SERVER="python tests/fixtures/csv_mcp_server.py"  # stdio 명령, sse/http는 URL
ADK_CC_MCP_SERVER_NAME=csv          # 도구 prefix → mcp__csv__*
ADK_CC_MCP_TRANSPORT=stdio          # stdio | sse | http
```

**정적 서버 여러 개 (단일 테넌트, 서버 다수)** — 모든 사용자에게 동일한 JSON 파일:
...
`McpServerConfig` 객체의 JSON 배열로, 부팅 시 로드되어 위의 단일 `ADK_CC_MCP_SERVER`와 병합됩니다(`server_name` 중복은 경고와 함께 제거). 장애 격리: 잘못된 파일이나 항목 하나는 로깅 후 건너뛰며 부팅을 막지 않습니다. 인증이 필요한 서버는 `credential_key`가 bearer 토큰을 담은 **env 변수 이름**을 가리킵니다(부팅 시점에는 credential store가 없음). 변수가 없으면 경고와 함께 비인증으로 연결됩니다.

**테넌트별 서버 (멀티테넌트, 사용자별 상이)** — 레지스트리. `ADK_CC_TENANT_REGISTRY_DIR`을 설정하고 테넌트별 `mcp.json`을 저장하세요. `TenantMcpToolset`이 invocation마다 활성 테넌트의 설정에서 서버를 해석하고(hot-reload), 자격 증명은 credential provider에서 치환되며, admin HTTP API로 런타임에 서버를 추가/삭제할 수 있습니다. 단일 테넌트 배포에서는 하나의 tenant_id를 사용해도 됩니다(dev에서는 기본 `"local"`).
