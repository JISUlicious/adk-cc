...
Covers:
  ...
  - Large file (>50 lines): default returns 50 with has_more=True.
...
def test_large_file_default_caps_at_50_lines() -> None:
    """File with 5000 lines: default (limit=50) reads lines 1..50,
    has_more=True. Default chosen to be small on purpose — 50 lines is
    well under any reasonable LLM context budget for a single read; the
    model bumps `limit` or paginates with `offset` when it actually
    needs more."""
    ...
    try:
        ...
        os.unlink(path)
    print("OK test_large_file_default_caps_at_50_lines")
...
def main() -> None:
    ...
    test_large_file_default_caps_at_50_lines()
