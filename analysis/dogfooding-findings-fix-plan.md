...
| F2c | Failed zero-output turns leave duplicate user messages in history | Medium | **FIXED** (P4: id-guarded `delete_last_event` on the file session service; broker prunes the orphan on the next turn — Retry or fresh message; UI e2e proves exactly one copy survives) |
